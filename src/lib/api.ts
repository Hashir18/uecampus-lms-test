const defaultLocalBase = (() => {
  ...
  if (host === "localhost" || host === "127.0.0.1") {
    return "http://localhost/uecampus-lms-main/backend/public/api";
  }
  return "/api";
})();
...
function candidateBases() {
  ...
  add("http://127.0.0.1/uecampus-lms-main/backend/public/api");
  add("http://localhost/uecampus-lms-main/backend/public/api");
  add("/api");
  ...
  [8080, 8086].forEach((port) => {
    ...
    add(`http://localhost:${port}/api`);
    add(`http://127.0.0.1:${port}/api`);
  });
