...
export function RightSidebar() {
  ...
  const fetchStats = async () => {
    try {
      ...
      // Prefer today's completed tasks; otherwise fall back to all-time completed
      const tasksCount = todayProgress?.length || completedProgress.length;

      // Rough study time: 30m per completed item
      const totalMinutes = (completedProgress?.length || 0) * 30;
