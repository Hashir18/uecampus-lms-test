...
class AssignmentController {
    public static function index(): void {
        ...
        // Only admins/teachers may query deadlines for another user
    ...
    // Returns total allowed attempts for the current user (base + extra)
    public static function attempts(string $id): void {
        $uid = require_user();
        $pdo = get_pdo();
        $stmtBase = $pdo->prepare('SELECT attempts FROM assignments WHERE id = :id');
        $stmtBase->execute([':id' => $id]);
        $base = (int) ($stmtBase->fetchColumn() ?: 0);
        $base = $base ?: 2;
        $stmt = $pdo->prepare('SELECT extra_attempts FROM assignment_extra_attempts WHERE assignment_id = :aid AND user_id = :uid LIMIT 1');
        $stmt->execute([':aid' => $id, ':uid' => $uid]);
        $extra = (int) ($stmt->fetchColumn() ?: 0);
        json_response([
            'attempts' => $base + $extra,
            'base_attempts' => $base,
